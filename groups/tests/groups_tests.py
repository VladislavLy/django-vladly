...
def test_list_of_groups():
    x = Client().post("/create-group/", data={'subject': 'Music', 'ratio_of_students': 2})
    assert x.status_code == 302
    response = Client().get('/list-of-groups/')
...
def test_delete_group():
    x = Client().post("/create-group/", data={'id': 1, 'subject': 'Music', 'ratio_of_students': 2})
    assert Group.objects.count() == 1
...
@pytest.mark.django_db
def test_post_edit_group():
    y = Client().post("/create-group/", data={'id': 1, 'subject': 'Music', 'ratio_of_students': 2})
    x = Client().post("/edit-group/1/", data={'id': 1, 'subject': 'Music', 'ratio_of_students': 2})
    ...
    response = Client().get('/edit-group/1/')
...
@pytest.mark.django_db
def test_list_of_groups_querry():
    x = Client().post("/create-group/", data={'subject': 'Music', 'ratio_of_students': 2})
    assert x.status_code == 302
    response = Client().get('/list-of-groups/?subject=Music')
    assert response.status_code == 200
    assert Group.objects.count() == 1
